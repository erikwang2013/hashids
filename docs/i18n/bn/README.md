...
- **Salt খালি রাখা মানে কোনো সুরক্ষা নেই**: খালি salt-এ এনকোড করা ফলাফল অনুমানযোগ্য (`encode(1)`, `encode(2)`… ক্রম আগেই বলে দেওয়া যায়)। `env('HASHIDS_SALT', '')` লিখলে এনভায়রনমেন্ট ভেরিয়েবল সেট না থাকলেও কোনো এরর বা ওয়ার্নিং আসে না,
  চুপচাপ খালি salt-এ নেমে যায় — লাইভে দেওয়ার আগে salt সেট আছে কি না নিশ্চিত করুন।
- **মেমোরিতে-থাকা ফ্রেমওয়ার্কে (Webman / Hyperf) কনফিগ বদলাতে প্রসেস রিস্টার্ট লাগে**: `HashidsManager` তৈরি হওয়ার সময়ই কনফিগ স্ন্যাপশট নেয় ও কানেকশন স্থায়ীভাবে ক্যাশে করে,
  তাই `config/hashids.php` বদলালে (বা কনফিগ সেন্টার থেকে salt বদলালে) সাথে সাথে কার্যকর হয় না — `reload` বা রিস্টার্টের পর হয়।
...
> **রানটাইম এনভায়রনমেন্ট**: নিচের স্তরের `hashids/hashids`-এর জন্য `ext-bcmath` বা `ext-gmp` (দুটোর একটি) **অবশ্যই** থাকতে হবে, না হলে প্রথম এনকোডেই ছোঁড়ে
> `RuntimeException: Missing math extension for Hashids`। এই দুটি এক্সটেনশন `hashids/hashids`-এ শুধু `suggest`-এ আছে,
> আর Composer 2 আর suggest প্রিন্ট করে না — ফলে স্লিম ইমেজে (যেমন `php:8.3-fpm-alpine`) ইনস্টলের সময় কোনো ইঙ্গিতই মেলে না, ধরা পড়ে শুধু রানটাইমে,
> আর Hashids ব্যবহার করা প্রতিটি রিকোয়েস্ট 500 দেয়। এই প্যাকেজের `composer.json`-এর `suggest`-এ দুটি কী যোগ করা হয়েছে, তবু এক্সটেনশন চালু আছে কি না আপনি নিশ্চিত করুন।
